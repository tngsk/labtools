...
def plot_summary(df, within, dv):
    """平均値と標準誤差を棒グラフで描画"""
    stats = (
        df.groupby(within)[dv].agg(mean="mean", std="std", count="count").reset_index()
    )
    ...
    fig = px.bar(
        ...
        y="mean",
        error_y="sem",
        title=f"Mean and Standard Error: {dv}",
        labels={"mean": dv},
    )
...
def plot_sphericity_check(df, subject, within, dv):
    ...
    fig.update_layout(
        title="Sphericity Check (Equality of Variance of Differences)", height=450
    )
    return fig
...
def run_rm_anova_analysis(
    ...
    aov = pg.rm_anova(
        data=df, dv=dv_col, within=within_col, subject=subject_col, correction=True
    )
    ...
    ph_bonf = pg.pairwise_tests(
        data=df, dv=dv_col, within=within_col, subject=subject_col, padjust="bonf"
    )
...
if __name__ == "__main__":
    ...
    run_rm_anova_analysis(
        dummy_df, subject_col="Subject_ID", within_col="Condition", dv_col="Score"
    )
